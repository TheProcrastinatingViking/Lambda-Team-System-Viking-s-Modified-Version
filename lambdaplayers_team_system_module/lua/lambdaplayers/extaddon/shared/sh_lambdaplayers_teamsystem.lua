...
adConquestMode = CreateLambdaConvar( "lambdaplayers_teamsystem_ad_conquest", 0, true, false, false, "If enabled, KOTH/AD uses conquest rules (tickets drain instead of score rising).", 0, 1, { name = "Enable Conquest Rules", type = "Bool", category = "Team System - KOTH/AD" } )
...
local ctfMinDefenders = CreateLambdaConvar( "lambdaplayers_teamsystem_ctf_min_defenders", 1, true, false, false, "Minimum Lambda Players per team that will prefer defending home flag when it is at home.", 0, 16, { name = "Defender Count", type = "Slider", decimals = 0, category = "Team System - CTF" } )
...
local ctfDefendBoostWhenFlagTaken = CreateLambdaConvar( "lambdaplayers_teamsystem_ctf_defend_boost_taken", 35, true, false, false, "How vigilant/aggressive should flag defenders be when the flag is not at home (higher values can cause the CTF gamemode to be very difficult, be prepared).", 0, 200, { name = "Defense Aggro When Flag Taken", type = "Slider", decimals = 0, category = "Team System - CTF" } )
...
local kdUsePoints = CreateLambdaConvar( "lambdaplayers_teamsystem_kd_usekothpoints", 0, true, false, false, "If enabled, kills will be confirmed only when they're delivered to one of KOTH points of team.", 0, 1, { name = "Pickups Use KOTH Points", type = "Bool", category = "Team System - KD" } )
...
local kdLambdaSeekTags = CreateLambdaConvar( "lambdaplayers_teamsystem_kd_lambdaseek", 1, true, false, false, "If enabled, Lambdas will seek out Kill Confirm tags (I would definitely have this enabled to stop matches from taking so long).", 0, 1, { name = "Should Lambda Players Collect KD Tags", type = "Bool", category = "Team System - KD" } )
